...
class SpammerBot(Thread):
    ...
    def run(self):
        ...
        try:
            ...
            wx.CallAfter(pub.sendMessage, 'updateProgress', None)
...
class SpammerPanel(wx.Panel):
    ...
    def InitGUI(self):
        ...
        self.emailsFilenamePath = wx.TextCtrl(self, wx.TE_READONLY)
        self.emailsFilenamePath.SetEditable(False)
        ...
        self.Bind(wx.EVT_BUTTON, self.OnChooseFileClicked, chooseFileButton)
        self.sendMeButton = wx.Button(self, size=(200, -1), label="Send %s emails" % self.emails)
        self.Bind(wx.EVT_BUTTON, self.OnSendMeClicked, self.sendMeButton)
        ...
        sizer.Add(self.emailsFilenamePath, 1, wx.ALL | wx.LEFT, 5)
    ...
    def OnChooseFileClicked(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            self.emailsFilenamePath.SetValue(dialog.GetPath())
        dialog.Destroy()
    ...
    def OnSendMeClicked(self, event):

        if self.emails == 0 or self.emailsFilenamePath.GetValue() == "":

            msg = "Choose CSV file and number of emails to send!"
            wx.MessageBox(msg, 'Warning', wx.OK | wx.ICON_WARNING)

        else:
            recipientsList = self.BuildRecipientsList(self.emailsFilenamePath.GetValue())[:self.emails]
            ...
            if self.emails > len(recipientsList):
                ...
                for i in range(0, self.emails):
                    ...
                    #     sb = SpammerBot(self.emailSubject.GetValue(), self.emailContent.GetValue(), recipient)
                    #     sb.start()
    ...
    def CleanUpGUI(self):
        self.count = 0
        self.emails = 0
        self.emailsFilenamePath.SetValue('')
